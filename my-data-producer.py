...
#attention: this is for python2, will not work for python3
#py -2 my-data-producer.py AAPL stock-analyzer 192.168.99.100:9092
...
def fetch_price(producer, symbol):
    ...
    :param stock: stock object to retrive price
    :return: None
    """
    #symbol = stock.get_name()
    logger.debug('Start to fetch stock price for %s', symbol)
    try:
        #stock.refresh()
        # timestamp = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%dT%H:%MZ')
        #timestamp = stock.get_trade_datetime()
        timestamp = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%dT%H:%MZ')
        #price = stock.get_price()
        price = random.randint(30, 120)
        payload = ('[{"StockSymbol":"%s","LastTradePrice":%s,"LastTradeDateTime":"%s"}]' % (symbol, price, timestamp)).encode('utf-8')
